...
for (wheatKey in names(Kw_list)) {
  for (mixKey in names(Ks_list)) {
    ...
    # Run Scenario 1
    ST1Models <- runS1(trait = "pycnidiaPerCm2Leaf",
                       Kw = Kw_list[[wheatKey]],
                       Kmix = Ks_list[[mixKey]],
    ...
    for (mix in unique(adjusted_phenotype$Strain)) {
      ST2strategy <- runS2(trait = "pycnidiaPerCm2Leaf",
                           Kw = Kw_list[[wheatKey]],
                           Kmix = Ks_list[[mixKey]],
                           phenotype = adjusted_phenotype,
                           genoW = genotype_wheat,
      ...
      weightedModel2 <- runS2(trait = "pycnidiaPerCm2Leaf",
                              Kw = Kw_list[[wheatKey]],
                              Kmix = Ks_list[[mixKey]],
                              phenotype = adjusted_phenotype,
                              genoW = genotype_wheat,
    ...
    for (mix in unique(adjusted_phenotype$Strain)) {
      ST3Models <- run_S3(trait = "pycnidiaPerCm2Leaf",
                          Kw = Kw_list[[wheatKey]],
                          Kmix = Ks_list[[mixKey]],
                          phenotype = adjusted_phenotype, 
                          sMix = mix,
      ...
      weightedModel3 <- run_S3(trait = "pycnidiaPerCm2Leaf",
                               Kw = Kw_list[[wheatKey]],
                               Kmix = Ks_list[[mixKey]],
                               phenotype = adjusted_phenotype, 
                               sMix = mix,
...
dir.create("data/modified_data/cv_leaf", recursive = T)
save(allResults, wtest_lines, file = paste0("data/modified_data/cv_leaf/iter_", iter, ".Rdata"))
